...
class Decks
{
    ...
    public function saveDeck ($user, $deck, $decklist_id, $name, $description, $tags, $mwl_id, $content, $source_deck)
    {
        ...
        if($mwl_id) {
            $mwl = $this->doctrine->getRepository('NetrunnerdbBuilderBundle:Mwl')->find($mwl_id);
            if ($mwl)
                $deck->setMwl($mwl);
        }
    ...
    {
        return $this->doctrine->getRepository('NetrunnerdbBuilderBundle:Deckchange')->findBy(array('deck' => $deck, 'saved' => FALSE));
    }
