...
use Netrunnerdb\BuilderBundle\Entity\Mwl;
...
class Judge
{
	...
	public function getInfluenceCostOfCard($slot, $slots, Card $identity)
	{
		...
		if($card->getKeywords() && strpos($card->getKeywords(), 'Alliance') !== FALSE) {
			...
			$targets = array_filter($arraySlots, function ($potentialTarget) use ($card) {
				return $potentialTarget->getCard()->getFaction()->getId() === $card->getFaction()->getId()
				&& strpos($potentialTarget->getCard()->getKeywords(), 'Alliance') === FALSE;
			});
	...
	public function getSpentInfluence(Decklist $decklist)
	{
		...
		/* @var $slot \Netrunnerdb\BuilderBundle\Entity\Decklistslot */
		foreach($decklist->getSlots() as $slot) {
	...
	public function computeLegality(Legality $legality)
	{
		...
		// quick access table to mwl penalties
		$mwlTable = array();
		/* @var $mwlslot \Netrunnerdb\BuilderBundle\Entity\Mwlslot */
		foreach($mwlslots as $mwlslot) {
			$mwlTable[$mwlslot->getCard()->getCode()] = $mwlslot->getPenalty();
		}
		...
		foreach($decklist->getSlots() as $slot) {
			...
			if(array_key_exists($cardCode, $mwlTable)) {
				$influencePenalty += $slot->getQuantity() * $mwlTable[$cardCode];
			}
		}
