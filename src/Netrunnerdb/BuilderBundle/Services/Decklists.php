...
class Decklists
{
    ...
    public function find ($start = 0, $limit = 30, Request $request)
    {
        ...
        $cardRepository = $this->doctrine->getRepository('NetrunnerdbCardsBundle:Card');
    ...
    {
    	$successors = $this->doctrine->getRepository('NetrunnerdbBuilderBundle:Decklist')->findBy(array('precedent' => $decklist));
    	foreach($successors as $successor)
    	{
    		/* @var $successor \Netrunnerdb\BuilderBundle\Entity\Decklist */
    		$successor->setPrecedent(null);
    	}
    
    	$children = $this->doctrine->getRepository('NetrunnerdbBuilderBundle:Deck')->findBy(array('parent' => $decklist));
    	foreach($children as $child)
    	{
    		/* @var $child \Netrunnerdb\BuilderBundle\Entity\Deck */
