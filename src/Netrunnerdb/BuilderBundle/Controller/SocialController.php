...
class SocialController extends Controller
{
    ...
    public function newAction (Request $request)
    {
        ...
        /* @var $judge \Netrunnerdb\BuilderBundle\Services\Judge */
        $judge = $this->get('judge');
        $analyse = $judge->analyse($deck->getSlots());
        if (is_string($analyse)) {
        ...
        */
        ...
        $tournament = $em->getRepository('NetrunnerdbBuilderBundle:Tournament')->find($tournament_id);
        ...
        $mwls = $em->getRepository('NetrunnerdbBuilderBundle:Mwl')->findAll();
        foreach($mwls as $mwl)
        {
    ...
    private function searchForm(Request $request)
    {
        ...
        $list_cycles = $this->get('doctrine')->getRepository('NetrunnerdbCardsBundle:Cycle')->findBy(array(), array("position" => "ASC"));
        ...
        $list_mwl = $em->getRepository('NetrunnerdbBuilderBundle:Mwl')->findBy(array(), array('dateStart' => 'DESC'));
    ...
    public function hidecommentAction($comment_id, $hidden, Request $request)
    {
        ...
        $comment = $em->getRepository('NetrunnerdbBuilderBundle:Comment')->find($comment_id);
        if(!$comment) {
	...
	*/
    ...
    public function profileAction ($user_id, $user_name, $page, Request $request)
    {
        ...
        $decklists = $em->getRepository('NetrunnerdbBuilderBundle:Decklist')->findBy(array('user' => $user));
        $nbdecklists = count($decklists);

        $reviews = $em->getRepository('NetrunnerdbBuilderBundle:Review')->findBy(array('user' => $user));
    ...
    public function followAction($user_id, Request $request)
    {
    	...
    	/* who is followed */
    	$following = $this->getDoctrine()->getManager()->getRepository('NetrunnerdbUserBundle:User')->find($user_id);
    ...
    public function activityAction ($days, Request $request)
    {
    	...
    	return $this->render('NetrunnerdbBuilderBundle:Activity:activity.html.twig',
    			array(
    					'pagetitle' => 'Activity',
