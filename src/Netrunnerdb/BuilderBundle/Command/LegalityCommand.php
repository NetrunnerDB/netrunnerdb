...
class LegalityCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
    	...
    	$mwl = $entityManager->getRepository('NetrunnerdbBuilderBundle:Mwl')->find($mwl_id);
    	...
    	if($decklist_id) 
    	{
    		...
    		$legality = $entityManager->getRepository('NetrunnerdbBuilderBundle:Legality')->findOneBy(['mwl' => $mwl, 'decklist' => $decklist]);
    		if(!$legality)
    		{
    	...
    	$countDql = "SELECT COUNT(d) 
					...
					WHERE NOT EXISTS (
						SELECT l 
						FROM NetrunnerdbBuilderBundle:Legality l
